...
def read_in_data(
    ...
    gene_dictionary = pd.Series(gene_def_df.gene_name.values, index=gene_def_df.gene_id)
    ...
    merged_df = merged_df.drop(columns=['label', 'IC50', 'cell line',
                                        'TCGA classification', 'Tissue', 'Tissue sub-type'])

    merged_df_counts = merged_df.astype('float64')
    merged_df_grouped = merged_df_counts.groupby(by=merged_df_counts.columns,
                                                 axis=1).mean()
